...
from data.image_folder import make_fruxel_dataset
from PIL import Image


class FruxelDataset(BaseDataset):
    def initialize(self, opt):
        print('FruxelDataset')
        self.opt = opt
        self.root = opt.dataroot
        self.dir_AB = os.path.join(opt.dataroot, opt.phase)
        self.AB_paths = make_fruxel_dataset(self.dir_AB)
    ...
    def __getitem__(self, index):
        ...
        B = np.load(B_path)
        #B = cv2.resize(B, dsize=(self.opt.loadSize, self.opt.loadSize), interpolation=cv2.INTER_NEAREST) 

        #B = np.reshape(B, (self.opt.loadSize, self.opt.loadSize, 1))
        B = np.clip(B, 0.0, 1.0).astype('float')
